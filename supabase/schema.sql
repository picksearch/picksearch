-- PickSearch Database Schema for Supabase
...
CREATE TABLE IF NOT EXISTS profiles (
  ...
  referred_by TEXT,
  search_coins INTEGER DEFAULT 0,
...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (id, email, full_name, search_coins)
  VALUES (
    NEW.id,
    NEW.email,
    COALESCE(NEW.raw_user_meta_data->>'full_name', NEW.raw_user_meta_data->>'name', ''),
    3  -- New user gets 3 coins
  );
  RETURN NEW;
END;
...
CREATE TABLE IF NOT EXISTS questions (
  ...
  question_type TEXT NOT NULL CHECK (question_type IN (
    ...
    'branching_choice', 'simple_branch', 'choice_with_other',
    'numeric_rating', 'likert_scale', 'image_choice', 'image_banner',
    'phone_number'
  )),
  options JSONB,
  image_urls JSONB,
