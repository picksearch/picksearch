...
export default function MySurveys() {
  ...
  const { data: surveys = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      let result = [];
      if (user.role === 'admin') {
        result = await Survey.list('-created_date');
      } else {
        result = await Survey.filter({ created_by: user.email }, '-created_date');
      }
  ...
  const statusConfig = {
    ...
    pending_media: { label: '설문대기', color: 'bg-yellow-100 text-yellow-700', icon: AlertCircle },
    media_review: { label: '설문대기', color: 'bg-yellow-100 text-yellow-700', icon: AlertCircle },
  ...
  const getTargetSummary = (survey) => {
    ...
    if (survey.target_settings?.cells?.[0]?.targets) {
      const targets = survey.target_settings.cells[0].targets;
      const parts = [];
  ...
  const getStatusGroup = (status) => {
    ...
    if (status === 'pending_media' || status === 'media_review' || status === 'rejected') return 'pending';
              ...
              return (
                <motion.div
                  ...
                  className="group">
                    ...
                    survey.status === 'live' ? 'bg-green-500' :
                    ...
                    survey.status === 'closed' ? 'bg-blue-500' : 'bg-orange-400'}`
                      ...
                      <div className="p-6 pb-5">
                        ...
                        <div className="mb-3 pr-10">
                          ...
                          <Badge className="bg-red-100 text-red-700 border-0 px-2.5 py-1 rounded-lg text-[11px] font-bold shadow-sm">
                            ...
                            <Badge className="bg-gray-50 text-gray-500 border-0 px-2.5 py-1 rounded-lg text-[11px] font-medium">
                          ...
                          <div className="text-sm text-gray-500">
                            {format(new Date(survey.created_date), 'yyyy-MM-dd HH:mm')}
                          </div>
                        </div>
                      ...
                      <div className="bg-gray-50/50 rounded-2xl p-5 border border-gray-100 mb-5">
                            <div className="grid grid-cols-2 gap-y-5 gap-x-4">
                              <div>
                                ...
                                <div className="text-sm font-bold text-gray-700 tracking-tight leading-tight">
                                  {survey.start_date ? format(new Date(survey.start_date), 'yyyy.MM.dd') : '-'} ~ 
                                  <br />
                                  {survey.end_date ? format(new Date(survey.end_date), 'yyyy.MM.dd') : '-'}
                                </div>
                              </div>
                        ...
                        <Link to={survey.survey_type === 'free' ? `${createPageUrl('CreateFreeSurvey')}?draft=${survey.id}` : `${createPageUrl('CreateSurvey')}?draft=${survey.id}`} className="w-full">
                              <Button className="w-full bg-[#3182F6] hover:bg-[#1B64DA] text-white rounded-xl h-11 font-bold shadow-lg shadow-blue-100 transition-all active:scale-[0.98]">
                                <Edit2 className="w-4 h-4 mr-2" />
                                이어서 작성하기
                              </Button>
                            </Link>
                          ...
                          {(survey.status === 'pending_media' || survey.status === 'media_review' || survey.status === 'scheduled') &&
                        ...
                        <div className="w-full flex gap-2">
                              <Button
                            onClick={() => copyToClipboard(getSurveyUrl(survey))}
                            variant="outline"
                            className="flex-1 border-2 border-emerald-100 hover:bg-emerald-50 text-emerald-600 rounded-xl h-11 font-bold">

                                <Copy className="w-4 h-4 mr-2" />
                                URL 복사
                              </Button>
                              <Link to={`${createPageUrl('FreeSurveyResults')}?id=${survey.id}`} className="flex-1">
                                <Button className="w-full bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl h-11 font-bold">
                                  <BarChart3 className="w-4 h-4 mr-2" />
                                  실시간 응답
                                </Button>
                              </Link>
                              <Button
                            onClick={() => {
                              if (confirm('설문을 종료하시겠습니까? 종료 후에는 더 이상 응답을 받을 수 없습니다.')) {
                                closeSurveyMutation.mutate(survey.id);
                              }
                            }}
                            variant="outline"
                            className="border-2 border-gray-200 hover:bg-gray-50 text-gray-600 rounded-xl h-11 font-bold px-3">
                        ...
                        <Link to={`${createPageUrl('FreeSurveyResults')}?id=${survey.id}`} className="w-full">
                              <Button className="w-full bg-white border-2 border-emerald-100 hover:bg-emerald-50 text-emerald-600 rounded-xl h-11 font-bold transition-all shadow-sm hover:shadow-md active:scale-[0.98]">
                                <BarChart3 className="w-4 h-4 mr-2" />
                                결과 보기
                              </Button>
                            </Link>
