...
export default function TakeSurvey() {
  ...
  useEffect(() => {
    ...
    if (savedAnswer) {
      if (currentQuestion.question_type === 'short_answer' || currentQuestion.question_type === 'phone_number') {
  ...
  const handleBranchingChoice = async (questionId, selectedOption) => {
    const newAnswers = {
      ...answers,
    ...
    const childQuestions = allQuestions.filter(
      q => q.parent_question_id === questionId && q.parent_branch_option === selectedOption
    ...
    if (childQuestions.length > 0) {
      ...
      if (currentQ?.branch_end_type === 'end_survey') {
        await completeSurvey(answerArray);
      } else {
        await handleBranchEnd(currentQ);
      }
    }
  ...
  const handleMultipleChoiceAnswer = async (questionId, answer) => {
    if (currentQuestion.question_type === 'branching_choice') {
      await handleBranchingChoice(questionId, answer);
      return;
    }

    // 분기형 질문 처리
    if (currentQuestion.question_type === 'simple_branch') {
      setIsProcessing(true);
      ...
      if (responseId && !isCompletingRef.current && survey?.status !== 'preview') {
        updateResponseMutation.mutate({ 
          answers: answerArray, 
          status: 'in_progress' 
        });
      ...
      // branch_targets에서 이동할 질문 번호 확인
      const targetQuestionNum = currentQuestion.branch_targets?.[answer];
      if (targetQuestionNum) {
        const targetIndex = rootQuestions.findIndex((_, idx) => idx + 1 === parseInt(targetQuestionNum));
        if (targetIndex !== -1 && targetIndex < rootQuestions.length) {
        ...
        <div key={currentQuestion?.id || 'loading'}>
            <Card className="bg-white rounded-3xl shadow-2xl overflow-hidden">
              ...
              <div className="flex flex-col max-h-[calc(100vh-200px)]">
                <div className="sticky top-0 bg-white z-10 p-5 pb-3 border-b border-gray-50">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    {currentQuestion.question_type === 'multiple_choice' ? '객관식' : 
                     currentQuestion.question_type === 'simple_branch' ? '분기형' :
                     ...
                     currentQuestion.question_type === 'phone_number' ? '전화번호' :
                     ...
                     <span className="text-xs text-emerald-600 font-medium">🔀 분기형</span>
                     )}
                     ...
                     {currentQuestion.question_type === 'phone_number' && (
                      <span className="text-xs text-emerald-600 font-medium">📱 전화번호를 입력하세요</span>
                     )}
                ...
                {(currentQuestion.question_type === 'multiple_choice' || currentQuestion.question_type === 'simple_branch' || currentQuestion.question_type === 'branching_choice') ? (
                  ...
                  <div className="space-y-3">
                    <div className="bg-emerald-50 rounded-xl p-3 border border-emerald-200 mb-3">
                      <p className="text-xs text-emerald-700 font-medium flex items-center gap-1">
                        <span>⚠️</span>
                        <span>허위 입력 시 리워드가 지급되지 않습니다</span>
                      </p>
                    </div>
                    <div className="relative">
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          setShortAnswerText(value.slice(0, 11));
                        }}
                        placeholder="010-0000-0000"
                        className="border-2 border-emerald-200 focus:border-emerald-500 rounded-xl h-11 text-sm px-4"
                        onKeyPress={(e) => {
                    ...
                    <Button
                      ...
                      disabled={!shortAnswerText.trim() || shortAnswerText.length < 10 || isProcessing}
                      className="w-full bg-gradient-to-r from-emerald-500 to-green-500 text-white shadow-lg rounded-xl h-11 text-sm font-bold disabled:opacity-50"
                    >
                      {isProcessing && <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin mr-2" />}
                      {(currentQuestionIndex === currentQuestionList.length - 1 && branchPath.length === 0) ? (
                        <>
