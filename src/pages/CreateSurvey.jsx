...
export default function CreateSurvey() {
  ...
  const loadAIGeneratedSurvey = () => {
    try {
      ...
      if (stored) {
        ...
        setPurpose(combinedPurpose || data.purpose || "");
        setQuestions(data.questions || []);
        ...
        if (data.target_settings) {
          try {
            if (Array.isArray(data.target_settings)) {
              setTargetSettings(data.target_settings);
            }
          } catch (e) {
            console.error('Failed to parse target_settings:', e);
          }
        }
  ...
  const loadCopiedSurvey = () => {
    try {
      ...
      if (stored) {
        ...
        if (data.target_settings) {
          setTargetSettings(Array.isArray(data.target_settings) ? data.target_settings : data.target_settings.cells || null);
        }
        ...
        setUseLandingPage(data.use_landing_page || false);
  ...
  const loadDraft = async (surveyId) => {
    try {
      ...
      if (surveys.length > 0) {
        ...
        // purpose에서 usagePurpose 분리 (저장 시 함께 저장된 경우)
        const purposeText = survey.purpose || "";
        const usagePurposeMatch = purposeText.match(/\n\n\[결과 활용 목적\]\n(.+)$/s);
        if (usagePurposeMatch) {
          setUsagePurpose(usagePurposeMatch[1].trim());
          setPurpose(purposeText.replace(/\n\n\[결과 활용 목적\]\n.+$/s, '').trim());
        } else {
          setPurpose(purposeText);
        }

        setDraftSurveyId(survey.id);
        setUseLandingPage(survey.use_landing_page || false);
        setLandingPageUrl(survey.landing_page_url || "");
        setSlotCount(survey.slot_count || 1);

        if (survey.start_date) {
          const savedStart = new Date(survey.start_date);
          const minStart = calculateMinStartDate();
          if (isBefore(startOfDay(savedStart), startOfDay(minStart))) {
            ...
            if (survey.end_date) setEndDate(new Date(survey.end_date));
          }
        }

        if (survey.target_settings) {
          const loadedTargets = Array.isArray(survey.target_settings) ?
          survey.target_settings :
          survey.target_settings?.cells || null;
        ...
        if (survey.target_settings?.customAppText) {
          setCustomAppText(survey.target_settings.customAppText);
          setEnableAppTargets(true);
        }
        if (survey.target_settings?.customLocationText) {
          setCustomLocationText(survey.target_settings.customLocationText);
  ...
  const addQuestion = (type) => {
    const newQuestion = {
      ...
      question_text: '',
      question_type: type,
      options: type === 'multiple_choice' || type === 'multiple_select' || type === 'ranking' || type === 'simple_branch' || type === 'choice_with_other' ? ['', ''] :
      type === 'likert_scale' ? ['', '', '', '', ''] : [],
      ...
      branch_targets: type === 'simple_branch' ? {} : undefined,
...
`;
  ...
  const canProceedFromStep6 = questions.length > 0 && questions.every((q) => {
    ...
    if (q.question_type === 'multiple_choice' || q.question_type === 'multiple_select' || q.question_type === 'ranking' || q.question_type === 'choice_with_other' || q.question_type === 'simple_branch') {
      return q.options && q.options.length >= 2 && q.options.every((o) => o && o.trim());
    }
    if (q.question_type === 'likert_scale') {
      return q.options && q.options.length === 5 && q.options.every((o) => o && o.trim());
    }
...
설문 만들기
              ...
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                onClick={() => navigate(createPageUrl('CreateFreeSurvey'))}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left flex-1">
                    <h3 className="font-bold text-lg mb-1">무료 설문 생성</h3>
            ...
            <CardContent className="p-6 space-y-4">
              ...
              'bg-gray-50 border-gray-200 hover:bg-gray-100'}`
                ...
                <div className="flex items-center gap-3">
                  ...
                  enableAdditionalTargets ? 'bg-blue-100' : 'bg-gray-200'}`
                  }>
              ...
              'bg-gray-50 border-gray-200 hover:bg-gray-100'}`
                ...
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  enableAppTargets ? 'bg-blue-100' : 'bg-gray-200'}`
                  }>
              ...
              'bg-gray-50 border-gray-200 hover:bg-gray-100'}`
                ...
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  enableLocationTargets ? 'bg-blue-100' : 'bg-gray-200'}`
                  }>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={async () => {
                  ...
                  try {
                    // Create temporary preview survey
                    const previewSurvey = await Survey.create({
                      title: title || '미리보기',
                      description: description || '',
                      secret_key: `preview_${Date.now()}`,
                      completion_secret_code: 'preview',
                      status: 'preview',
                      total_cost: 0,
                      survey_type: 'preview'
                    });
                    ...
                    window.open(`${window.location.origin}${createPageUrl('TakeSurvey')}?key=${previewSurvey.secret_key}`, '_blank');
                  } catch (error) {
            ...
            <CardContent className="p-6 space-y-3">
              <AnimatePresence>
                {questions.map((question, index) => {
                  ...
                  console.log('🎨 렌더링:', { questionId: question.id, isHighlighted, highlightedQuestionId });
                  ...
                  return (
                    ...
                    <Card
                        className={`bg-white rounded-2xl shadow-sm transition-all cursor-pointer ${
                        isHighlighted ?
                        'border-2 border-blue-400 shadow-lg shadow-blue-100 ring-2 ring-blue-100' :
                        'border border-gray-100'}`
                        }
                        onClick={() => {
                          console.log('🖱️ 카드 클릭, 하이라이트 제거');
                        ...
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge className={
                              ...
                              question.question_type === 'simple_branch' ? 'bg-sky-100 text-sky-700 border-0' :
                              ...
                              {question.question_type === 'multiple_choice' ? '객관식' :
                                ...
                                question.question_type === 'simple_branch' ? '분기형' :
                      ...
                      {(question.question_type === 'multiple_choice' || question.question_type === 'multiple_select' || question.question_type === 'ranking' || question.question_type === 'simple_branch' || question.question_type === 'choice_with_other') &&
    ...
    <div className="min-h-screen bg-gray-50 pb-32">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 z-50">
          <div className="max-w-2xl mx-auto flex gap-3">
            ...
            className={`flex-[2] h-14 text-base rounded-xl font-bold ${
