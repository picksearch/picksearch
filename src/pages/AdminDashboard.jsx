...
import { Survey, Question, CreditTransaction } from "@/api/entities";
...
export default function AdminDashboard() {
  ...
  const approveCreditMutation = useMutation({
    mutationFn: async (transaction) => {
      // 거래 상태 업데이트
      await CreditTransaction.update(transaction.id, {
        status: 'confirmed'
      });
  ...
  const rejectCreditMutation = useMutation({
    mutationFn: async (transactionId) => {
      await CreditTransaction.update(transactionId, {
        status: 'rejected'
      });
    },
    onSuccess: () => {
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-500">
                          요청일: {format(new Date(transaction.created_date), 'yyyy.MM.dd HH:mm')}
                        </div>
                      </div>
